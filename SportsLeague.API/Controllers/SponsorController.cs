...

{
...

    {
        ...
        if (sponsor == null)
            ...
            return NotFound(new { message = $"Sponsor with the {id} not found" });
...

    {
...

        {
            ...
            await _sponsorService.RegisterSponsorAsync(dto.TournamentId, id, dto.ContractAmount);

            return Ok(new { message = "Sponsor succesfully added" });
    ...
    public async Task<ActionResult<IEnumerable<TournamentResponseDTO>>> GetTournaments(int id)
    {
        try
        {
            var tournaments = await _sponsorService.GetSponsorByIdWithTournamentAsync(id);
    ...
    [HttpDelete("{id}/tournaments")]

    public async Task<ActionResult> UnboundSponsor(int id, UnboundSponsorRequestDTO dto)
    {

        try
        {

            await _sponsorService.UnboundSponsorAsync(dto.tournamentId, id);

            return Ok(new { message = "Sponsor succesfully removed" });
